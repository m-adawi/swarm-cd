    ...
    <>
      {filteredStatuses.length === 0 ? (
        ...
        filteredStatuses.map((item, index) => (
          <StatusCard key={index} name={item.Name} error={item.Error} revision={item.Revision} repoURL={item.RepoURL} templatePath={item.TemplatePath} composePath={item.ComposePath} />
        ))
      )}
    </>
  )
